...
public class ScheduleDetail extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 添付ファイルの削除
		// なぜか研究室のサーバーでファイルアップロードをしたところ、parserequestで止まってしまうので、ローカルではcommonsを使っているが、サーバーではtomcat
		// ので動いたので、そのようにする
		// 研究室のサーバの方ではビルドパスをcommonslangとmailはプロジェクトフォルダ直下のlibフォルダからで良いが、
		// fileuploadやjdbcはtomcat内のlibフォルダにjarファイルを入れてそこから参照しないといけない
		// 研究室のサーバでは、データベース名やホスト名、パスワードが異なるので、meta-inf内のcontextファイルを書き換えたり、
		// notifmanagerの方では、それぞれのtable.java内のjdbcのところを書き換えるべし

		// キーパーソンは、ファイルを変えるのも勿論だが、sqlのtargetテーブルの最後にkeyというカラムを追加する。
		// その時、keyはおそらくmysqlの予約語であるので、バッククォーテーションで囲うべし
		// また、追加するときにset defaultで初期値をNULLじゃなくて0とかにしとくべし
